        ...
        <div class="row align-items-center justify-content-center min-vh-100">
            <div class="col-12 col-sm-10 col-md-6 col-lg-4 col-xl-4">
                <div class="logo text-center mb-5 mx-auto">
                    <a href="{{ url('/website') }}" title="الصفحة الرئيسية">
                        <img src="{{url('/')}}/assets/website/assets/images/logo.png" alt="#" class="img-fluid">
                    </a>
                    <h1 class="mt-3 text-dark font-weight-bold h6">قم بالتسجيل في حساب بطاقة الخاص بك</h1>
                </div><!-- logo -->
                <div class="login_form">
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div class="form-group mt-5 position-relative has-link">
                            <label for="username">اسم بطاقتك</label>
                            @if(session('betakaName') != '')
                                @foreach(session('betakaName') as  $betakaName)
                                    <input id="username" type="text" class="auth_input @error('name') is-invalid @enderror" name="name" value="{{ $betakaName }}" required autocomplete="off" autofocus>
                                @endforeach
                            @elseif(session('betakaName') == '')
                                <input id="username" type="text" class="auth_input @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required autocomplete="off" autofocus>
                            @endif
                            <div class="prepend-text">betaka.com/</div>
                            @error('name')
                            <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="form-group mt-5 position-relative has-link">
                            <label for="username">اسم المستخدم</label>
                                <input id="username" type="text" class="auth_input @error('userName') is-invalid @enderror" name="userName" value="{{ old('userName') }}" required autocomplete="off" autofocus>
                            @error('userName')
                            <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                ...
                <div class="auth__footer mt-4 d-flex align-items-center justify-content-center">
                    <a href="{{ route('login') }}" title="#" class="h6 text-dark">هل لديك حساب ؟</a>
                </div><!-- auth__footer -->
            </div><!-- col -->
        </div><!-- row -->
    </div><!-- container -->
